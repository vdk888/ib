#!/usr/bin/env python3
"""
Portfolio Optimizer for Uncle Stock Screeners
Calculates optimal portfolio allocations based on quarterly returns, volatilities, and correlations
Uses scipy optimization to maximize Sharpe ratio
"""
...
def extract_quarterly_returns(universe_data: Dict[str, Any]) -> pd.DataFrame:
    """
    Extract quarterly returns for each screener from universe data
    
    Returns:
        DataFrame with columns for each screener and quarterly returns as rows
    """
...
def calculate_portfolio_stats(weights: np.ndarray, returns: pd.DataFrame, risk_free_rate: float = 0.02/4) -> Tuple[float, float, float]:
    """
    Calculate portfolio statistics
    
    Args:
        weights: Portfolio weights
        returns: DataFrame of quarterly returns
...
def negative_sharpe_ratio(weights: np.ndarray, returns: pd.DataFrame, risk_free_rate: float = 0.02/4) -> float:
    """Objective function: negative Sharpe ratio (for minimization)"""
    _, _, sharpe = calculate_portfolio_stats(weights, returns, risk_free_rate)
    return -sharpe  # Negative because we want to maximize Sharpe ratio

def optimize_portfolio(returns: pd.DataFrame, risk_free_rate: float = 0.02/4) -> Dict[str, Any]:
    """
    ...
    expected_return, volatility, sharpe_ratio = calculate_portfolio_stats(optimal_weights, returns, risk_free_rate)
...
def update_universe_with_portfolio(universe_data: Dict[str, Any], portfolio_results: Dict[str, Any]) -> None:
...
def display_portfolio_results(portfolio_results: Dict[str, Any]) -> None:
...
def main():
    ...
    try:
        ...
        returns_df = extract_quarterly_returns(universe_data)
        ...
        portfolio_results = optimize_portfolio(returns_df)
        ...
        update_universe_with_portfolio(universe_data, portfolio_results)
